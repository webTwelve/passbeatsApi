...
def success(*, data: Union[list, dict, str, ResultProxy], ) -> Response:
...
def ok(*, message: str, data: Union[list, dict, str, ResultProxy]) -> Response:
    # data = type_conversion(data)
    return ORJSONResponse(
        status_code=status.HTTP_200_OK,
        content={
...
# 数据类型转换
def type_conversion(data: Union[list, dict, str, ResultProxy]) -> Union[
    ...
    if isinstance(data, ResultProxy):
